...
*Confirming the Windows Security Events connector is active and streaming logs.*
...
*Disabling the Windows Defender Firewall to allow traffic to reach the OS for log generation.*
...
Intentionally exposing the VM to the public internet to attract brute-force traffic.*
...
*Managing public/private IP interfaces to distinguish between legitimate administrative traffic and malicious probes.*
...
*KQL query results identifying high-volume brute-force attempts from multiple international IPs.*
...
*The logic flow: Sentinel Trigger -> Get IP Entities -> Create NSG Deny Rule.*
...
*Confirmation that the 'PB-Block-Attacker-IP' playbook was triggered successfully on Incident ID 2.*
...
*The NSG now contains a 'Deny' rule for the attacker's IP (60.250.231.169), created automatically by the SOAR playbook.*
